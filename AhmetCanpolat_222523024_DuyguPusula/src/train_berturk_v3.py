...
from src.config import TEXT_COLUMN, LABEL_COLUMN, RANDOM_STATE, METRICS_DIR, FIGURES_DIR


MODEL_NAME = "dbmdz/bert-base-turkish-cased"
DATA_PATH = Path("data/processed/turkish_tweet_emotion_augmented_v3.csv")
MODEL_OUTPUT_DIR = Path("models/berturk_v3")
...
def save_confusion_matrix(actuals, predictions, labels, id2label):
    ...
    path = FIGURES_DIR / "berturk_v3_confusion_matrix.png"
...
def save_history_plot(history):
    ...
    path = FIGURES_DIR / "berturk_v3_training_history.png"
    plt.savefig(path, dpi=220)
    plt.close()
...
def save_model(model, tokenizer, label2id, id2label, metrics):
    ...
    with open(MODEL_OUTPUT_DIR / "model_info.json", "w", encoding="utf-8") as file:
...
def train():
    ...
    pd.DataFrame(history).to_csv(METRICS_DIR / "berturk_v3_training_history.csv", index=False, encoding="utf-8-sig")

    with open(METRICS_DIR / "berturk_v3_metrics.json", "w", encoding="utf-8") as file:
        json.dump(metrics, file, ensure_ascii=False, indent=4)

    with open(METRICS_DIR / "berturk_v3_classification_report.json", "w", encoding="utf-8") as file:
        json.dump(report, file, ensure_ascii=False, indent=4)

    print()
    print("BERTurk v3 eğitimi tamamlandı.")
